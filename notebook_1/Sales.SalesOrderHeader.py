...
#pegando arquivos 
df = spark.read.option("header", "true").option("inferSchema", "true").option("delimiter", ";").csv("/mnt/blobthai/Sales.SalesOrderHeader.csv")
...
dfc = df.withColumn("ModifiedDate",col("ModifiedDate").cast(DateType()))\
        .withColumn("ShipDate",
...
dfp.loc[dfp['PurchaseOrderNumber']== "NULL", "PurchaseOrderNumber"] = "NaoInformado"


# COMMAND ----------

dfp.loc[dfp['CurrencyRateID']== "NULL", "CurrencyRateID"] = "NaoInformado"

# COMMAND ----------

dfp.loc[dfp['CreditCardApprovalCode']== "NULL", "CreditCardApprovalCode"] = "NaoInformado"

# COMMAND ----------

dfp.loc[dfp['Comment']== "NULL", "CommentCreditCardApprovalCode"] = "NaoInformado"


# COMMAND ----------

dfp['TotalDue'] = dfp['TotalDue'].str.replace(',','.').astype(float)
